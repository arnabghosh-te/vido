            ...
            <div className="w-64 bg-white border-r border-gray-200 flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto p-2">
                    {documents.map(doc => (
                        <div 
                            key={doc.id}
                            onClick={() => { setSelectedDocument(doc); setMessages([]); }}
                    ...
                    {documents.length === 0 && (
                        <p className="text-gray-500 text-sm text-center mt-4">No documents uploaded yet.</p>
                    )}
                </div>
            </div>
                ...
                {selectedDocument ? (
                    <>
                        <div className="p-4 border-b border-gray-200 bg-white">
                            <h3 className="font-semibold text-gray-800">Chatting with: {selectedDocument.originalName}</h3>
                        </div>
                            ...
                            {messages.length === 0 ? (
                                ...
                                messages.map((msg, idx) => (
                                    <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`max-w-[70%] p-3 rounded-lg ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white border border-gray-200 text-gray-800 rounded-bl-none'}`}>
